...
namespace VoxelSystem {
	...
	public class GPUVoxelizer {
        ...
        public static RenderTexture BuildTexture3D(ComputeShader voxelizer, GPUVoxelData data, Texture2D texture, RenderTextureFormat format, FilterMode filterMode)
        {
            ...
            voxelizer.SetTexture(kernel.Index, kColorTextureKey, Texture2D.whiteTexture);
        ...
        static RenderTexture CreateTexture3D(GPUVoxelData data, RenderTextureFormat format, FilterMode filterMode)
        {
            ...
            // 複製紋理的每一層到新的紋理中
            RenderTexture copiedTexture = new RenderTexture(data.Height, data.Width, 0, format, RenderTextureReadWrite.Default);
            copiedTexture.dimension = TextureDimension.Tex2DArray;
            copiedTexture.volumeDepth = data.Depth;
            copiedTexture.enableRandomWrite = true;
            copiedTexture.filterMode = filterMode;
            copiedTexture.wrapMode = TextureWrapMode.Clamp;
            copiedTexture.Create();
            for (int z = 0; z < data.Depth; z++)
            {
                Graphics.CopyTexture(texture, z, copiedTexture, z);
            }

            // 旋轉紋理
            RenderTexture rotatedTexture = RotateTexture3D(copiedTexture);
            UnityEngine.Object.DestroyImmediate(copiedTexture);
            UnityEngine.Object.DestroyImmediate(texture);
            return rotatedTexture;
        }


        static RenderTexture RotateTexture3D(RenderTexture src)
        {
            RenderTexture dst = new RenderTexture(src.height, src.width, src.volumeDepth, src.format);
            dst.dimension = TextureDimension.Tex3D;
            dst.enableRandomWrite = true;
            dst.filterMode = src.filterMode;
            dst.wrapMode = src.wrapMode;
            dst.depth = 0; // Set depth to 0 to create a texture without depth
            dst.Create();

            Matrix4x4 rotationMatrix = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0f, 90f, 0f), Vector3.one);
            Material copyMaterial = new Material(Shader.Find("Modular Bricks/Pipes/PBL"));
            copyMaterial.SetMatrix("_RotationMatrix", rotationMatrix);
            copyMaterial.SetInt("_CopyZ", 1);
        ...
        static RenderTexture GetTextureLayer(RenderTexture tex3D, int z)
        {
            var layer = new RenderTexture(tex3D.width, tex3D.height, 0, tex3D.format, RenderTextureReadWrite.Default);
            layer.dimension = TextureDimension.Tex2D;
            layer.useMipMap = false;
            layer.filterMode = tex3D.filterMode;
            layer.wrapMode = tex3D.wrapMode;
            layer.Create();
            layer.volumeDepth = 1;
            layer.depth = 0;
            Graphics.CopyTexture(tex3D, 0, z, layer, 0, 0);
