...
Shader "Modular Bricks/Pipes/PBL_Right" {
	Properties{
		...
		_GlobalVarlengthx("Global Var Length X", Float) = 0
		_GlobalVarlengthy("Global Var Length Y", Float) = 0
		_GlobalVarlengthz("Global Var Length Z", Float) = 0
		_GlobalVarminx("Global Var Min X", Float) = 0
		_GlobalVarminy("Global Var Min Y", Float) = 0
		_GlobalVarminz("Global Var Min Z", Float) = 0
	...
	SubShader{
		...
		Pass{
			...
			half _Radius;
			half _Height;
			uniform float _GlobalVarlengthx;
			uniform float _GlobalVarlengthy;
			uniform float _GlobalVarlengthz;
			uniform float _GlobalVarminx;
			uniform float _GlobalVarminy;
			uniform float _GlobalVarminz;
			...
			void geom(triangle Input p[3], inout TriangleStream<Interpolators> triStream)
			{
				...
				if ((triangleNormal / 3).y > 0.98)
				{
					...
					//float4 circle_start = d_mid_rot90 * _Radius * -1.0f;
					//float4 circle_start = d_mid_rot90 * _Radius * float4(0.0f, -1.0f, 0.0f, 0.0f);
					...
					//if(d_mid.z <= _GlobalVarminz + (_GlobalVarlengthz/2)){
					//left
					/*vertices_up[0] = mid_ip + float4(circle_start.x, 0.0f-0.4, circle_start.z+0.4, 0.0f);
					vertices_down[0] = mid_ip + float4(circle_start.x, _Height / 10 -0.4, circle_start.z+0.4, 0.0f);
					...
					for (int i = 0; i < 7; i++)
					{
						...
						vertices_up[i] = mid_ip + float4(point_2d.x, point_2d.y-0.4,0.0f+0.4, 0.0f);
						vertices_down[i] = mid_ip + float4(point_2d.x, point_2d.y-0.4, _Height / 10+0.4, 0.0f);
						normals[i] = normal;
					}*/
					...
					//else {
					//right
					vertices_up[0] = mid_ip + float4(circle_start.x, 0.0f-0.4, circle_start.z-0.5, 0.0f);
					vertices_down[0] = mid_ip + float4(circle_start.x, _Height / 10 -0.4, circle_start.z-0.5, 0.0f);
					normals[0] = float3(circle_start.x, 0.0f, circle_start.z);
					...
					for (int i = 0; i < 7; i++)
					{
						...
						float3 normal = float3(point_2d.x, 0.0f, point_2d.y);
						normal = normalize(normal * float3(1, 0, -1)); // flip the z-axis
						vertices_up[i] = mid_ip + float4(point_2d.x, point_2d.y-0.4,0.0f-0.5, 0.0f);
						vertices_down[i] = mid_ip + float4(point_2d.x, point_2d.y-0.4, _Height / 10-0.5, 0.0f);
						normals[i] = normal;
					}
					//}
		...
		Pass{
			...
			void geom(triangle Input p[3], inout TriangleStream<Interpolators> triStream)
			{
				...
				if ((triangleNormal / 3).y > 0.98)
				{
					...
					vertices_up[0] = mid_ip + float4(circle_start.x, _Height / 10, circle_start.z+20, 0.0f);
					...
					*/
					...
					for (int y = 0; y < 7; y++)
					{
						triStream.Append(CreateInterpolator(vertices_down[y], normals[y], p[0].texcoord0));
						triStream.Append(CreateInterpolator(vertices_up[y], normals[y], p[0].texcoord0));
					}
				}
			}
			...
			void geom(triangle Input p[3], inout TriangleStream<Interpolators> triStream)
			{
				...
				if ((triangleNormal / 3).y > 0.98)
				{
					...
					*/
					...
					for (int k = 0; k < 7; k++)
					{
						triStream.Append(CreateV2F(vertices_down[k], normals[k]));
						triStream.Append(CreateV2F(vertices_up[k], normals[k]));
					}
				}
			}
