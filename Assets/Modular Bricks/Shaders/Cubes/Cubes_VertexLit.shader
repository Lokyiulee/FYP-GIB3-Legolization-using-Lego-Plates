...
Shader "Modular Bricks/Cubes/VertexLit"
{
	Properties
	{
		...
		Pass
		{
			...
			void GS_Main(triangle GS_INPUT p[3], inout TriangleStream<FS_INPUT> triStream)
			{
				...
				if ((triangleNormal / 3).y > 0.98)
				{
					...
					// desenam baza
					// prima fata
					pIn.pos = UnityObjectToClipPos(vertices_down[0]);
					worldNormal = UnityObjectToWorldNormal(normal_45);
					nl = max(0, dot(worldNormal, _WorldSpaceLightPos0.xyz));
					pIn.diff = nl * _LightColor0;
					pIn.ambient = ShadeSH9(half4(worldNormal, 1));
					pIn.tex0 = p[0].tex0;
					...
					pIn.pos = UnityObjectToClipPos(vertices_down[1]);
					worldNormal = UnityObjectToWorldNormal(normal_45);
					nl = max(0, dot(worldNormal, _WorldSpaceLightPos0.xyz));
					pIn.diff = nl * _LightColor0;
					pIn.ambient = ShadeSH9(half4(worldNormal, 1));
					...
					// a doua fata
					pIn.pos = UnityObjectToClipPos(vertices_down[1]);
					worldNormal = UnityObjectToWorldNormal(normal_135);
					nl = max(0, dot(worldNormal, _WorldSpaceLightPos0.xyz));
					pIn.diff = nl * _LightColor0;
					pIn.ambient = ShadeSH9(half4(worldNormal, 1));
					pIn.tex0 = p[0].tex0;
					...
					pIn.pos = UnityObjectToClipPos(vertices_down[2]);
					worldNormal = UnityObjectToWorldNormal(normal_135);
					nl = max(0, dot(worldNormal, _WorldSpaceLightPos0.xyz));
					pIn.diff = nl * _LightColor0;
					pIn.ambient = ShadeSH9(half4(worldNormal, 1));
