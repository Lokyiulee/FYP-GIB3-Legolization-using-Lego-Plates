...
Shader "Modular Bricks/Cubes/Diffuse Texture Transparent" {
	Properties{
	...
	SubShader{
		...
		Pass{
			...
			void geom(triangle Input p[3], inout TriangleStream<Interpolators> triStream)
			{
				...
				if ((triangleNormal / 3).y > 0.98)
				{
					...
					float4 vertices_up[3];
					float4 vertices_down[3];
					float3 normal_45;
					float3 normal_135;
					...
					float2 norm = RotateVector(float2(d_mid_rot90.x, d_mid_rot90.z), 45);
					normal_45 = float3(norm.x, 0.0f, norm.y);
					normal_135 = float3(-normal_45.z, 0.0f, normal_45.x);

					for (int i = 1; i < 3; i++)
					{
						float2 point_2d = RotateVector(float2(circle_start.x, circle_start.z), i * 90);
					...
					// desenam baza
					// prima fata
					triStream.Append(CreateInterpolator(vertices_down[0], normal_45, p[0].texcoord0));
					triStream.Append(CreateInterpolator(vertices_up[0], normal_45, p[0].texcoord0));
					triStream.Append(CreateInterpolator(vertices_down[1], normal_45, p[0].texcoord0));
					triStream.Append(CreateInterpolator(vertices_up[1], normal_45, p[0].texcoord0));
					triStream.RestartStrip();
					// a doua fata
					triStream.Append(CreateInterpolator(vertices_down[1], normal_135, p[0].texcoord0));
					triStream.Append(CreateInterpolator(vertices_up[1], normal_135, p[0].texcoord0));
					triStream.Append(CreateInterpolator(vertices_down[2], normal_135, p[0].texcoord0));
					triStream.Append(CreateInterpolator(vertices_up[2], normal_135, p[0].texcoord0));
				}
			}
		...
		Pass{
			...
			float4 frag(Interpolators i) : COLOR{
				...
				/// Final Color:
				float3 finalColor = diffuse;
				fixed4 finalRGBA = fixed4(finalColor * (_Color.a*_MainTex_var.a),0);
				UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
				return finalRGBA;
			}
			ENDCG
		...
		Pass
		{
			...
			void geom(triangle Input p[3], inout TriangleStream<Interpolators> triStream)
			{
				...
				if ((triangleNormal / 3).y > 0.98)
				{
					...
					// desenam baza
					// prima fata
					triStream.Append(CreateV2F(vertices_down[0], normal_45));
					triStream.Append(CreateV2F(vertices_up[0], normal_45));
					triStream.Append(CreateV2F(vertices_down[1], normal_45));
					triStream.Append(CreateV2F(vertices_up[1], normal_45));
					triStream.RestartStrip();
					// a doua fata
					triStream.Append(CreateV2F(vertices_down[1], normal_45));
					triStream.Append(CreateV2F(vertices_up[1], normal_45));
					triStream.Append(CreateV2F(vertices_down[2], normal_45));
					triStream.Append(CreateV2F(vertices_up[2], normal_45));
				}
			}
